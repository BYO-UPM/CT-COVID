...
def hdbscan_bboxes(boxes, min_cluster_size=3, min_samples=None, cluster_selection_epsilon=0.5, use_sizes=False, n_boxes_threshold=None):
    ...
    if use_sizes:
        data = np.array([
            [(box[0] + box[2]) / 2, (box[1] + box[3]) / 2, box[2] - box[0], box[3] - box[1]] for box in boxes_normalized])
    else:
...
def plot_object_detection_df(df, sample_size, *columns, figsize=None, samplimg_method=None, print_overlap_percentage=False, save_dir=None):
    ...
    for i in range(1, sample_size+1):
        ...
        plt.title(file_name, fontsize=11)

        for j, column in enumerate(columns, start=1):
            bboxes = row[column]
            image_with_bboxes = draw_image_and_bboxes(img, bboxes, (255,0,0), "covid")
            bbox_name = column if column != "original_bbox" else "original_bboxes"
            fig.add_subplot(rows, sample_size, j*sample_size + i)
            bbox_img_title = f"{bbox_name}: {len(bboxes)} boxes "
            if print_overlap_percentage:
                uncovered_percentage = row[f"%uncovered_mask_{column}"]
                bbox_img_title = bbox_img_title + "{:.2f}".format((1-uncovered_percentage)*100) + "%"
            plt.title(bbox_img_title, fontsize=10)
